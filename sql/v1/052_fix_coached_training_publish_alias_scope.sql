-- V1.52: forward repair for V1.51's target-table alias scope in the seat update.
...
begin
  ...
  select s.id, s.title, s.starts_at, s.ends_at, s.session_type into v_session
  from public.lineup_boards b join public.sessions s on s.id = b.session_id
  where b.id = p_lineup_board_id for update of b, s;
  ...
  -- `ls` is the UPDATE target and is therefore correlated only in WHERE, not a FROM JOIN condition.
  ...
  for v_boat in
    ...
    from public.lineup_boats lb join public.boats b on b.id = lb.fleet_boat_id
  ...
  loop
    ...
    if exists (
      ...
      join public.profiles p on p.id = member.id join public.boats b on b.id = v_boat.fleet_boat_id
      where p.status <> 'active' or public.skill_level_to_clearance(p.skill_level) < b.required_clearance
    ...
    ) then raise exception 'A final lineup member is not eligible for %', v_boat.boat_name; end if;

    select r.id into v_target_reservation_id from public.reservations r
    ...
    foreach v_member_id in array v_boat.member_ids loop
      ...
      from public.reservation_crew rc join public.reservations r on r.id = rc.reservation_id join public.boats old_boat on old_boat.id = r.boat_id
      where rc.member_id = v_member_id and r.status = 'reserved'
        and tstzrange(r.start_time, r.end_time, '[)') && tstzrange(v_session.starts_at, v_session.ends_at, '[)') and r.boat_id <> v_boat.fleet_boat_id
      for update of r;
