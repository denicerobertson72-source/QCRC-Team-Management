...
export default async function TrainingBIPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [trainingGroup, defaultMonth] = await Promise.all([
    ...
    getNextProgramSessionMonth(["coached_training_beginner_intermediate"]),
  ]);
    ...
    <>
      ...
      <main className="stack">
        ...
        <div className="row calendar-toolbar">
          <div className="row">
            <Link href="/programs/training">Back to Groups</Link>
            <Link href={`/programs/training/beginner-intermediate?month=${month.prev}&${viewQuery}`}>Previous Month</Link>
            <Link href={`/programs/training/beginner-intermediate?month=${month.next}&${viewQuery}`}>Next Month</Link>
          </div>
          <div className="calendar-toggle">
            <Link
              href={`/programs/training/beginner-intermediate?${monthQuery}&view=list`}
              className={`button-link ${view === "list" ? "" : "button-link-secondary"}`.trim()}
            >
              List View
            </Link>
            <Link
              href={`/programs/training/beginner-intermediate?${monthQuery}&view=calendar`}
